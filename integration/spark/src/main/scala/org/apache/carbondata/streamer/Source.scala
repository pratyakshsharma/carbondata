...
abstract class Source {
  ...
  def prepareDSForAvroSourceAndMerge(
    ...
    val deserializerToConvertToCatalystRow = new AvroDeserializer(schema, sparkDataTypes)
    ...
    inputStream.inputDStream.asInstanceOf[DStream[GenericRecord]].foreachRDD { rdd =>
      val rowRDD = rdd.map { row =>
        encoder.fromRow(deserializerToConvertToCatalystRow
          .deserialize(row)
      ...
      val targetDs = inputStream.sparkSession
        ...
        .as("A")
      val sourceDS = inputStream.sparkSession.createDataFrame(rowRDD, sparkDataTypes).as("B")
