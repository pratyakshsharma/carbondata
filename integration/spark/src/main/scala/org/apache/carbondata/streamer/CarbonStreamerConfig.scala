...
class CarbonStreamerConfig() extends Serializable {
  ...
  @Parameter(names = Array("--deduplicate"),
    ...
    required = false)
  var deduplicateEnabled: Boolean = CarbonCommonConstants
    .CARBON_STREAMER_INSERT_DEDUPLICATE_DEFAULT
    .toBoolean
  ...
  @Parameter(names = Array("--combine-before-upsert"),
    ...
    required = false)
  var isCombineBeforeUpsert: Boolean = CarbonCommonConstants
    .CARBON_STREAMER_UPSERT_DEDUPLICATE_DEFAULT.toBoolean

  @Parameter(names = Array("--continuous-mode"),
    description = "If this property is set to true, the streamer tool will run as an ever running" +
                  " job, continuously ingesting data at regular intervals of time specified by " +
                  "--min-bath-interval",
    required = false)
  var isContinuousMode: Boolean = CarbonCommonConstants
    .CARBON_STREAMER_CONTINUOUS_MODE_DEFAULT
    .toBoolean
  ...
  def setConfigsToCarbonProperty(streamerConfig: CarbonStreamerConfig): Unit = {
    ...
    carbonPropertiesInstance.addProperty(CarbonCommonConstants.CARBON_STREAMER_UPSERT_DEDUPLICATE,
      streamerConfig.isCombineBeforeUpsert.toString)
    carbonPropertiesInstance.addProperty(CarbonCommonConstants.CARBON_STREAMER_CONTINUOUS_MODE,
      streamerConfig.isContinuousMode.toString)
